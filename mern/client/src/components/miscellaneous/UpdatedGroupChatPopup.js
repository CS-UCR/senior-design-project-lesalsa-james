...
const UpdatedGroupChatPopup = ({fetchAgain, setFetchAgain, fetchMessages }) => {
    ...
    const handleRemove = async(removeduser) => {
        if (selectedChat.groupAdmin._id !== user._id && removeduser !== user._id) {
            toast({
                title: "Error: Only Admin can remove",
                status: "error",
                duration: 3000,
                isClosable: true,
                position: "bottom",
    ...
    const handleAddUser = async (addeduser) => {
        if (selectedChat.users.find((u) => u._id === addeduser._id)) {
            toast({
                title: "Error: User Already in group",
                status: "error",
                duration: 3000,
                isClosable: true,
                position: "bottom",
        ...
        if (selectedChat.groupAdmin._id !== user._id) {
            toast({
                title: "Error: Only Admin can add",
                status: "error",
                duration: 3000,
                isClosable: true,
                position: "bottom",
    ...
    return (
        <>
          ...
          <Modal isOpen={isOpen} onClose={onClose}>
            <ModalOverlay />
            <ModalContent>
              <ModalHeader fontSize="32px" justifyContent = "center" d="flex"> {selectedChat.chatName}</ModalHeader>
              <ModalCloseButton />
              <ModalBody alignItems="center" d="flex" flexDir="column">
                    ...
                    <FormControl>
                        <Input placeholder="Add a user to this group chat" mb = {1} onChange={(e) => handleSearch(e.target.value)}/>
                    </FormControl>
                    {loading ? (
                        <Spinner size= "lg" />
                    ) : (
...
export default UpdatedGroupChatPopup;
